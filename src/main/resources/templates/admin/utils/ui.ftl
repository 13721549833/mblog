...
<#--    <#local pageNo = p.number + 1 />-->
    <#local pageNo = p.current />
    <#if p.total % p.size != 0>
        <#local totalPages = (p.total / p.size)?floor + 1 />
    <#else>
        <#local totalPages = (p.total / p.size)?floor />
    </#if>
...
<ul class="pagination no-margin pull-right">
    ...
    <#if (totalPages > totalNo)>
        <#if (pageNo <= span + 2)>
            <#list 1..totalNo1 as i>
                <@pagelink pageNo, i, cURL/>
            </#list>
            <@pagelink 0, 0, "#"/>
            <@pagelink pageNo, totalPages, cURL />
        <#elseif (pageNo > (totalPages - (span + 2)))>
            <@pagelink pageNo, 1, cURL />
            <@pagelink 0, 0, "#"/>
            <#local num = totalPages - totalNo + 2 />
            <#list num..totalPages as i>
                <@pagelink pageNo, i, cURL/>
            </#list>
        <#else>
            <@pagelink pageNo, 1, cURL />
